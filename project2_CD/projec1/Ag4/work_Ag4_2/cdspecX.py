...
# python cdspecX.py --prefix Ag4 --gauge length --origin COM --outdir ./work_Ag4_gauss
...
def main(prefix, gauge, origin, outdir, width, e_min, e_max, delta_e, png):
    ...
    mm_x = os.path.join(outdir, f'mm-{origin}-gauss_x.dat')
    mm_y = os.path.join(outdir, f'mm-{origin}-gauss_y.dat')
    mm_z = os.path.join(outdir, f'mm-{origin}-gauss_z.dat')
