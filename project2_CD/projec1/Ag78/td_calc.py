...
#$ gpaw -P 64 python td_calc.py --gpw ./work_Ag78/Ag78-full-cluster.gpw   --mode lcao --gauge length --kick z --dt_as 20 --nsteps 1600 --outdir ./work_Ag78
#gpaw -P 64 python td_calc.py --gpw ./work_GG-Ag2/GG-Ag2.gpw --mode lcao --gauge length --kick x --outdir ./work_GG-Ag2
...
def run_lcao(gpw, outdir, kick, gauge, dt_as, nsteps, origin, origin_shift):
    ...
    td = LCAOTDDFT(gpw, txt=os.path.join(outdir, f'td-{kick}.out'),parallel=dict(sl_auto=True, domain=8, augment_grids=True)
    )
...
def run_fd(gpw, outdir, kick, dt_as, nsteps):
    ...
    kick_strength['xyz'.index(kick)] = 1e-3
...
if __name__ == "__main__":
    ...
    ap.add_argument("--dt_as", type=float, default=20.0, help="步长 attoseconds")
