...
# gpaw -P 48 python gsrun.py -- --xyz ./Ag78-full-cluster.xyz   --mode lcao --basis dzp --nbands 2380 --outdir ./work_Ag78
...
def main(xyz, mode, basis, nbands, h, vacuum, outdir):
    ...
    calc = GPAW(
        ...
        xc='PBE',
        nbands=nbands,                     # 用下限 2370
        occupations=FermiDirac(0.10),      # ← 电子温度 0.10 eV，显著稳 SCF
        mixer=Mixer(0.05, 5, 40),         # ← 更小步长+更长历史，抑制振荡
        poissonsolver={'name':'MomentCorrectionPoissonSolver',
                       'poissonsolver':'fast','moment_corrections':1+3+5},
        convergence={'density': 1e-4, 'energy': 5e-5},  # ← 合理阈值，不必 1e-12
        txt=os.path.join(outdir, f'{name}-gs.out'),
        symmetry={'point_group': False},
        parallel=dict(sl_auto=True, band=6, domain=16,augment_grids=True),
    )
